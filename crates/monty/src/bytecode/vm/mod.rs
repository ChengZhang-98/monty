...
impl<'h, 'a, T: ResourceTracker> VM<'h, 'a, T> {
    ...
    pub fn snapshot(mut self) -> VMSnapshot {
        ...
        VMSnapshot {
            ...
            meta_stack: mem::take(&mut self.meta_stack),
            meta_globals: mem::take(&mut self.meta_globals),
            meta_exception_stack: mem::take(&mut self.meta_exception_stack),
            metadata_store: mem::take(&mut self.metadata_store),
        }
    }
    ...
    /// execution, without needing metadata. REPL callers prefer
    /// `take_globals_with_meta()`. Must be called before the VM is dropped,
    /// since `Drop` will clean up any remaining globals with `drop_with_heap`.
