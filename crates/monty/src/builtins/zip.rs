...
pub fn builtin_zip(vm: &mut VM<'_, '_, impl ResourceTracker>, args: ArgValues) -> RunResult<Value> {
    ...
    // Create iterators for each iterable. `defer_drop_mut!` guarantees every
    // iterator built so far is dropped on early return (e.g. if a later
    // `MontyIter::new` call fails).
    ...
    for (i, iterable) in positional.enumerate() {
        ...
        iterators.push(MontyIter::new(iterable, vm, meta)?);
    }
            ...
            if let Some((item, _meta)) = iter.for_next(vm)? {
                ...
                if strict {
                    ...
                    for (j, remaining) in iterators.iter_mut().enumerate().skip(1) {
                        if let Some((extra, _meta)) = remaining.for_next(vm)? {
