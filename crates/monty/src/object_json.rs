...
use crate::{
    ...
    object::{AnnotatedDictPairs, MontyObject},
    types::Type,
};
...
///
/// let obj = MontyObject::List(vec![MontyObject::Int(1).into(), MontyObject::String("x".into()).into()]);
...
impl Serialize for JsonMontyObject<'_> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        match self.0 {
            ...
            MontyObject::List(items) => serialize_annotated_seq(serializer, items),
            MontyObject::Dict(pairs) => serialize_annotated_dict(serializer, pairs),
            ...
            MontyObject::Tuple(items) => serialize_tagged_annotated_seq(serializer, "$tuple", items),
            MontyObject::Set(items) => serialize_tagged_annotated_seq(serializer, "$set", items),
            MontyObject::FrozenSet(items) => serialize_tagged_annotated_seq(serializer, "$frozenset", items),
            ...
            MontyObject::Repr { repr, .. } => serialize_tagged(serializer, "$repr", repr),
...
/// Serialize a slice of `AnnotatedObject`s as a JSON array. Metadata is
/// dropped (natural JSON form has no place for it); each `value` is wrapped
/// in `JsonMontyObject` so recursion continues with natural-form rules.
fn serialize_annotated_seq<S: Serializer>(serializer: S, items: &[AnnotatedObject]) -> Result<S::Ok, S::Error> {
    let mut seq = serializer.serialize_seq(Some(items.len()))?;
    for item in items {
        seq.serialize_element(&JsonMontyObject(&item.value))?;
    }
    seq.end()
}
...
/// `serialize_tagged_seq` for `AnnotatedObject` slices; strips metadata and
/// recurses via `JsonMontyObject` on each inner value.
fn serialize_tagged_annotated_seq<S: Serializer>(
    serializer: S,
    tag: &'static str,
    items: &[AnnotatedObject],
) -> Result<S::Ok, S::Error> {
    serialize_tagged(serializer, tag, &AnnotatedSeqBody(items))
}

/// Body shim so `AnnotatedObject` slices can reuse `serialize_tagged`.
struct AnnotatedSeqBody<'a>(&'a [AnnotatedObject]);

impl Serialize for AnnotatedSeqBody<'_> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        serialize_annotated_seq(serializer, self.0)
    }
}
...
/// them would be lossy.
/// `serialize_dict` for `AnnotatedDictPairs`; metadata is dropped and each key
/// / value is recursed via `JsonMontyObject`.
fn serialize_annotated_dict<S: Serializer>(serializer: S, pairs: &AnnotatedDictPairs) -> Result<S::Ok, S::Error> {
    let is_all_strings = pairs
        .into_iter()
        .all(|(k, _)| matches!(k.value, MontyObject::String(_)));
    if is_all_strings {
        let mut map = serializer.serialize_map(Some(pairs.len()))?;
        for (k, v) in pairs {
            let MontyObject::String(key) = &k.value else {
                unreachable!()
            };
            map.serialize_entry(key, &JsonMontyObject(&v.value))?;
        }
        map.end()
    } else {
        serialize_tagged(serializer, "$dict", &AnnotatedDictPairsBody(pairs))
    }
}

struct AnnotatedDictPairsBody<'a>(&'a AnnotatedDictPairs);

impl Serialize for AnnotatedDictPairsBody<'_> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut seq = serializer.serialize_seq(Some(self.0.len()))?;
        for (k, v) in self.0 {
            seq.serialize_element(&[JsonMontyObject(&k.value), JsonMontyObject(&v.value)])?;
        }
        seq.end()
    }
}
...
impl Serialize for FieldsBody<'_> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        for (name, value) in self.field_names.iter().zip(self.values) {
            map.serialize_entry(name, &JsonMontyObject(&value.value))?;
        }
        map.end()
    }
}
...
struct AttrsBody<'a>(&'a AnnotatedDictPairs);
