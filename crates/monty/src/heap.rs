...
pub struct HeapRead<'a, T: ?Sized> {
    value: NonNull<T>,
    /// Pointer to the `readers` counter in the owning `HeapEntry`.
    ///
...
impl<T: ?Sized> Drop for HeapRead<'_, T> {
    fn drop(&mut self) {
        // SAFETY: (DH) the readers pointer is valid for the lifetime of the HeapEntry,
...
impl<'a, T: ?Sized> HeapRead<'a, T> {
    ...
    pub fn get<'r, RT: ResourceTracker>(&self, _: &'r HeapReader<'a, RT>) -> &'r T {
        ...
        //  - The address of the `HeapEntry` never changes because entries are stored in
        ...
        //    `HeapEntry`) which guarantees the entry will never be freed by `dec_ref`
        //    or `collect_garbage` while this `HeapRead` exists.
        //  - The type of the `HeapEntry` can never change once allocated. This is
...
/// This wrapper containing `UnsafeCell` exists to allow for data inside of `HeapEntry`
