...
use monty::{AnnotatedDictPairs, AnnotatedObject, ExcType, JsonMontyObject, MontyObject, MontyRun};

fn to_json(obj: &MontyObject) -> String {
    serde_json::to_string(&JsonMontyObject(obj)).unwrap()
}

/// Wrap a `Vec<MontyObject>` into the `Vec<AnnotatedObject>` shape now required
/// by `MontyObject::List`/`Tuple`/`Set`/`FrozenSet`/namedtuple values. Metadata
/// is left empty — these tests exercise JSON serialization only.
fn ann(items: Vec<MontyObject>) -> Vec<AnnotatedObject> {
    items.into_iter().map(Into::into).collect()
}

/// Same idea for dict/attrs pairs: `Vec<(MontyObject, MontyObject)>` →
/// `AnnotatedDictPairs` with empty metadata.
fn ann_pairs(pairs: Vec<(MontyObject, MontyObject)>) -> AnnotatedDictPairs {
...
fn json_output_list_bare_array() {
    let obj = MontyObject::List(ann(vec![
        MontyObject::Int(1),
...
fn json_output_tuple_tagged() {
    let obj = MontyObject::Tuple(ann(vec![MontyObject::Int(1), MontyObject::String("two".into())]));
...
fn json_output_set_and_frozenset_tagged() {
    let s = MontyObject::Set(ann(vec![MontyObject::Int(1), MontyObject::Int(2)]));
    assert_snapshot!(to_json(&s), @r#"{"$set":[1,2]}"#);

    let fs = MontyObject::FrozenSet(ann(vec![MontyObject::Int(3)]));
...
fn json_output_namedtuple_fields_and_name() {
    ...
    let obj = MontyObject::NamedTuple {
        ...
        values: ann(vec![MontyObject::Int(1), MontyObject::Int(2)]),
    };
...
fn json_output_dataclass_nested() {
    ...
    let inner_attrs = ann_pairs(vec![(MontyObject::String("v".into()), MontyObject::Int(7))]);
    ...
    let outer_attrs = ann_pairs(vec![(MontyObject::String("inner".into()), inner)]);
